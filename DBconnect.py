...
def updateTask(data):
    ...
    if(user_date<end_date):
        ...
        if(user_data):
            ...
            dop_user_fio = data[0]
            dop_user_end_date = datetime.strptime(data[1], '%Y-%m-%d').date()
            ...
            if(dop_user_end_date<user_date):
                ...
                print(f'{dop_user_fio} Данный разработчик схожий по скиллам {task_user}. И он можете взять в работу с {dop_user_start_date} по {dop_user_end_date}')
